...
public class Tracer {
  ...
  public ScopedSpan startScopedSpanWithParent(String name, @Nullable TraceContext parent) {
    ...
    TraceContext context = newContextBuilder(parent, sampler).build();
    TraceContext decorated = propagationFactory.decorate(context);
    CurrentTraceContext.Scope scope = currentTraceContext.newScope(decorated);
    ScopedSpan result;
    if (!noop.get() && Boolean.TRUE.equals(decorated.sampled())) {
      result = new RealScopedSpan(decorated, scope, recorder, errorParser);
      recorder.name(context, name);
      recorder.start(context);
    } else {
      result = new NoopScopedSpan(decorated, scope);
    }
    return result;
  }
